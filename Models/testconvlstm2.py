...
def makeData(n_samples, ymd, n_flames=6, data_type="train_data"):
    input_images = []
    label_images = []
    image_size = [int(436 * 0.2), int(257 * 0.2)] # height, width
    for i in range(1, divmod(n_samples, 12)[0]+1):
        ...
        if len(row) == n_flames * 2:
            ...
            input_images.append(row[5:11])
            label_images.append(row[1:7])
...
seq = keras.Sequential(
    [
        ...
        layers.ConvLSTM2D(
            filters=40, kernel_size=(3, 3), padding="same", return_sequences=True, activation="relu", dropout=0.1
        ),
