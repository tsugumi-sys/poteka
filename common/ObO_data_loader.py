...
def load_all_data(params: List[str] = ["rain", "humidity", "temperature", "wind"]) -> Tuple[np.ndarray, np.ndarray]:
    tracemalloc.start()
...
def load_selected_data(params: List[str] = ["rain", "humidity", "temperature", "wind"]) -> Tuple[np.ndarray, np.ndarray]:
    tracemalloc.start()
